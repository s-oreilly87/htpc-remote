...
//   Back to using telnet-client (after trying telnet-stream)

//   Still using callback setup - going to try async/await next
 ...
 */
const DenonTelnetClient = function(ip) {
    this.params = {
        host: '192.168.1.101',
        port: 23,
        irs: '\r',  //'\r\n' ???
...
DenonTelnetClient.prototype.connect = async function() {
    ...
    //     console.log("Telnet ready: Starting to send from queue!")
    //     this.sendNextTelnetQueueItem()
    // })

    // default data listener for logging (doesnt send response)
    // will eventually use this for event listening maybe?
    ...
    try {
        const response = await this.connection.connect(this.params)
    } catch (err) {
...
DenonTelnetClient.prototype.setDataHandlerCallback = function(callback) {
    ...
    this.connection.once('data', handleData)

    console.log("data handler set up")
}

DenonTelnetClient.prototype.sendNextTelnetQueueItem = function() {
    console.log("sendNextTelnetQueueItem -  cmdQueue.length=" + this.cmdQueue.length)
    if (this.cmdQueue.length > 0) {
        if (!this.connection) {
            this.connect()
        } else {
            console.log("connection found - proceeding . . .")
            const item = this.cmdQueue.shift();
            this.setDataHandlerCallback(item.callback)
            console.log("data handler should be setup before this")
            console.log("sending cmd: " + item.cmd)
 ...
 */
DenonTelnetClient.prototype.addCmdToQueue = function(cmd, callback, waitfor=undefined) {
    console.log("denon.addCmdToQueue -  busy=" + this.busy)
 ...
 */
DenonTelnetClient.prototype.cmd = function(cmd, callback) {
    console.log("denon.cmd - calling addCmdToQueue")
 ...
 */
DenonTelnetClient.prototype.parseSimpleResponse = function(data, regexp) {
    let match
 ...
 */
DenonTelnetClient.prototype.getInput = function(callback, zone) {
 ...
 */
DenonTelnetClient.prototype.setInput = function(input, callback, zone) {
    const commandPrefix = (!zone || (zone === 'ZM')) ? 'SI' : zone;
 ...
 */
DenonTelnetClient.prototype.getMuteState = function(callback, zone) {
 ...
 */
DenonTelnetClient.prototype.setMuteState = function(muteState, callback, zone) {
    const commandPrefix = (!zone || (zone === 'ZM')) ? '' : zone;
 ...
 */
DenonTelnetClient.prototype.getVolume = function(callback, zone) {
 ...
 */
DenonTelnetClient.prototype.setVolume = function(volume, callback, zone) {
    const commandPrefix = (!zone || (zone === 'ZM')) ? 'MV' : zone;
 ...
 */
DenonTelnetClient.prototype.getZonePowerState = function(callback, zone) {
 ...
 */
DenonTelnetClient.prototype.setZonePowerState = function(powerState, callback, zone) {
    const commandPrefix = (!zone || (zone === 'ZM')) ? 'MV' : zone;
...
export default DenonTelnetClient
