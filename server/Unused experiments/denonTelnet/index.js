...
var MarantzDenonTelnet = function(ip) {
    this.connectionparams = {
        host: ip,
        port: 23,
        timeout: 2000, //  The RESPONSE should be sent within 200ms of receiving the request COMMAND. (plus network delay)
        sendTimeout: 300,
 ...
 */
MarantzDenonTelnet.prototype.sendNextTelnetCueItem = async function() {
    var mdt = this;
    if (this.cmdCue.length > 0) {
        if (!this.connection) {
            ...
            this.connection.on('timeout', () => {
                console.error("Connection timed out with no response.")
                mdt.connection.close()
            });

            try {
                await this.connection.connect(this.connectionparams)
            } catch (err) {
            ...
            var send_options = {timeout: (isRequestCommand ? this.connectionparams.sendTimeout : 10)};
            //var send_options = {timeout: this.connectionparams.sendTimeout};
...
//if (isRequestCommand) console.log(`\n\n\n${item.cmd}:${JSON.stringify(data)}`);
                ...
                //} else if (error && error.message === 'response not received') { // if it is no 'request COMMAND' the AVR will not respond
                    data = [];
                    error = null;
                }
...
MarantzDenonTelnet.prototype.cmd = function(cmd, callback) {
    ...
    var regexp = RegExp('(?:^|[\r])' + cmd.substring(0, 2) + '([A-Z ]+)');
...
MarantzDenonTelnet.prototype.setZonePowerState = function(powerState, callback, zone) {
    var commandPrefix = (!zone || (zone == 'ZM')) ? 'PW' : zone;

    this.telnet(commandPrefix + (powerState ? 'ON' : (zone && zone !== 'ZM' ? 'OFF' : 'STANDBY')), function(error, data) {
        if (!error) {
            callback(null, powerState);
        } else {
            callback(error);
...
/**
    Export.
*/
export default MarantzDenonTelnet;
