...
function VolumeControls({ remote }) {
    ...
    return(
        <div className="flex flex-col relative h-full w-full">
            {remote === REMOTE.DENON &&
                <div className={"absolute w-3/4 -top-12 right-0 flex justify-center items-center text-center self-center bg-gray-800 rounded-lg border-slate-700 p-1"}>
                        <span className={`text-teal-500 text-2xl ${dot_matrix.className} ${ denonState.loading ? 'opacity-50 animate-pulse' : '' }`}>
                            {denonState.MV}
                        </span>
                </div>
            }
