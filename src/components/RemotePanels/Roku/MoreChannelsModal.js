...
import {fetchRokuChannels, sendRokuLaunchCommand} from "@/utilities/http.js";
import {ROKU_APPS} from "@/utilities/constants.js";
import {FontAwesomeIcon} from "@fortawesome/react-fontawesome";
import {faXmark} from "@fortawesome/free-solid-svg-icons";
import {buttonPress} from "@/utilities/utils.js";

export default function MoreChannelsModal({isOpen, setIsOpen, moreChannels, fetchIcons, setPowerOn}) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setTimeout(() => {
                fetchIcons('extra-channel-button')
                        ...
                        <div className="flex min-h-full items-center justify-center p-4 text-center">
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-md relative transform overflow-visible rounded-2xl bg-violet-950 p-6 text-left align-middle shadow-xl transition-all -top-3">
                                    ...
                                    <div className={`grid grid-cols-3 grid-rows-2 gap-3 w-full ${iconsLoaded ? 'opacity-100': 'opacity-0'} transition-opacity duration-500 ease-in-out`}>
