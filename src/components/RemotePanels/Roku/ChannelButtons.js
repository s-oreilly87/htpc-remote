...
import {fetchRokuChannels, sendRokuLaunchCommand, sendRokuQuery} from "@/utilities/http"
...
function RokuChannels({ setPowerOn }) {
    ...
    const fetchChannels = async () => {
        ...
        const frontPageChannelIds = Object.values(ROKU_APPS.CHANNELS).map(channel => channel.id)

        const moreChannels = channels.filter(channel => !frontPageChannelIds.includes(channel.id))
    ...
    return (
        <>
            <MoreChannelsModal isOpen={moreChannelsModalOpen}
            ...
            <div id="roku-channels" className="w-full flex place-content-center gap-2">
                <div className={`grid grid-cols-4 grid-rows-2 gap-3 w-full ${iconsLoaded ? 'opacity-100': 'opacity-0'} transition duration-150 ease-in-out`}>
                    ...
                    <button className={"more-channels-button z-50"} onClick={handleClickMore}>
                        <FontAwesomeIcon icon={ faPlus } size={"2xl"} beatFade={true} color={"#461699"} />
                    </button>
                </div>
            </div>
        </>
