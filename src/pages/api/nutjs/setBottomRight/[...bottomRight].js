...
export default async function handleSetBottomRight(req, res) {
    ...
    setNutState({
        scaleFactor: {
            x: nutState.screenSize.width / (nutState.calibrationRange.xMax - nutState.calibrationRange.xMin),
            y: nutState.screenSize.height / (nutState.calibrationRange.yMax - nutState.calibrationRange.yMin)
        }
    })

    console.log(`NUTJS: Set scaleFactor to { x: ${nutState.scaleFactor.x}, y: ${nutState.scaleFactor.y} }`)
