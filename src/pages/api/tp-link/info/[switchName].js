...
export default async function handleInfo(req, res) {
  ...
  async function getSwitchInfo(switchName) {
    const lightSwitch = Object.values(LIGHTSWITCHES).find(
      (lightSwitch) => lightSwitch.id === switchName,
    );
  ...
  if (
    !switchName ||
    switchName === LIGHTSWITCHES.BEDROOM.id ||
    switchName === "all"
  ) {
